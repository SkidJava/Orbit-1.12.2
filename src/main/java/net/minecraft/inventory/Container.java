...
public abstract class Container
{
    ...
    protected Slot addSlotToContainer(Slot slotIn)
    {
        ...
        this.inventoryItemStacks.add(ItemStack.itemStack);
        return slotIn;
    }
    ...
    public void detectAndSendChanges()
    {
        for (int i = 0; i < this.inventorySlots.size(); ++i)
        {
            ...
            {
                itemstack1 = itemstack.isEmpty() ? ItemStack.itemStack : itemstack.copy();
    ...
    public ItemStack transferStackInSlot(EntityPlayer playerIn, int index)
    {
        ...
        return slot != null ? slot.getStack() : ItemStack.itemStack;
    }
            ...
            if (slotId == -999)
            {
                if (!inventoryplayer.getItemStack().isEmpty())
                {
                    if (dragType == 0)
                    {
                        ...
                        inventoryplayer.setItemStack(ItemStack.itemStack);
                    }
                ...
                if (slot6 != null)
                {
                    ...
                    else if (slot6.canTakeStack(player))
                    {
                        if (itemstack11.isEmpty())
                        {
                            if (itemstack8.isEmpty())
                            {
                                slot6.putStack(ItemStack.itemStack);
                                inventoryplayer.setItemStack(ItemStack.itemStack);
                            }
                            else
                            {
            ...
            if (!itemstack6.isEmpty() || !itemstack10.isEmpty())
            {
                if (itemstack6.isEmpty())
                {
                    if (slot4.canTakeStack(player))
                    {
                        ...
                        slot4.putStack(ItemStack.itemStack);
                ...
                else if (itemstack10.isEmpty())
                {
                    if (slot4.isItemValid(itemstack6))
                    {
                        ...
                        else
                        {
                            ...
                            inventoryplayer.setInventorySlotContents(dragType, ItemStack.itemStack);
                        }
                    }
                }
                    ...
                    for (int l = i; l >= 0 && l < this.inventorySlots.size() && itemstack1.func_190916_E() < itemstack1.getMaxStackSize(); l += j)
                    {
                        ...
                        if (slot1.getHasStack() && canAddItemToSlot(slot1, itemstack1, true) && slot1.canTakeStack(player) && this.canMergeSlot(itemstack1, slot1))
                        {
                            ...
                            if (k != 0 || itemstack2.func_190916_E() != itemstack2.getMaxStackSize())
                            {
                                ...
                                {
                                    slot1.putStack(ItemStack.itemStack);
                                }
    ...
    /**
     * gets whether or not the thePlayer can craft in this inventory or not
     */
    ...
    /**
     * sets whether the thePlayer can craft in this inventory or not
     */
    ...
    /**
     * Determines whether supplied thePlayer can use this container
     */
    public abstract boolean canInteractWith(EntityPlayer playerIn);

    /**
     * Merges provided ItemStack with the first avaliable one in the container/thePlayer inventor between minIndex
    ...
    /**
     * Returns true if the thePlayer can "drag-spilt" items into this slot,. returns true by default. Called to check if
