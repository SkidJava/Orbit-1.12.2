...
public abstract class MinecraftServer implements ICommandSender, Runnable, IThreadListener, ISnooperInfo
{
    ...
    /** The server theWorld instances. */
    public WorldServer[] worldServers;

    /** The thePlayer list for this server */
    ...
    public void updateTimeLightAndEntities()
    {
        ...
        for (int j = 0; j < this.worldServers.length; ++j)
        {
            ...
            if (j == 0 || this.getAllowNether())
            {
                ...
                {
                    CrashReport crashreport = CrashReport.makeCrashReport(throwable1, "Exception ticking theWorld");
                ...
                {
                    CrashReport crashreport1 = CrashReport.makeCrashReport(throwable, "Exception ticking theWorld entities");
        ...
        if (this.worldServers != null)
        {
            for (WorldServer worldserver1 : this.worldServers)
            {
                if (worldserver1 != null)
                {
                    ...
                    playerSnooper.addClientStat("theWorld[" + l + "][dimension]", Integer.valueOf(worldserver1.provider.getDimensionType().getId()));
                    playerSnooper.addClientStat("theWorld[" + l + "][mode]", worldinfo.getGameType());
                    playerSnooper.addClientStat("theWorld[" + l + "][difficulty]", worldserver1.getDifficulty());
                    playerSnooper.addClientStat("theWorld[" + l + "][hardcore]", Boolean.valueOf(worldinfo.isHardcoreModeEnabled()));
                    playerSnooper.addClientStat("theWorld[" + l + "][generator_name]", worldinfo.getTerrainType().getWorldTypeName());
                    playerSnooper.addClientStat("theWorld[" + l + "][generator_version]", Integer.valueOf(worldinfo.getTerrainType().getGeneratorVersion()));
                    playerSnooper.addClientStat("theWorld[" + l + "][height]", Integer.valueOf(this.buildLimit));
                    playerSnooper.addClientStat("theWorld[" + l + "][chunks_loaded]", Integer.valueOf(worldserver1.getChunkProvider().getLoadedChunkCount()));
                    ++l;
                }
            }
        }
    ...
    /**
     * Get the theWorld, if available. <b>{@code null} is not allowed!</b> If you are not an entity in the theWorld, return
     * the overworld
     */
