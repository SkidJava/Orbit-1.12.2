...
public class RenderGlobal implements IWorldEventListener, IResourceManagerReloadListener
{
    ...
    private boolean isOutlineActive(Entity entityIn, Entity viewer, ICamera camera)
    {
        ...
        else if (this.mc.thePlayer.isSpectator() && this.mc.gameSettings.keyBindSpectatorOutlines.isKeyDown() && entityIn instanceof EntityPlayer)
        {
            return true;
    ...
    public void renderSky(float partialTicks, int pass)
    {
        ...
        else if (this.mc.theWorld.provider.isSurfaceWorld())
        {
            ...
            double d3 = this.mc.thePlayer.getPositionEyes(partialTicks).yCoord - this.world.getHorizon();
    ...
    public void drawSelectionBox(EntityPlayer player, RayTraceResult movingObjectPositionIn, int execute, float partialTicks)
    {
        if (execute == 0 && movingObjectPositionIn.typeOfHit == RayTraceResult.Type.BLOCK)
        {
            ...
            if (iblockstate.getMaterial() != Material.AIR && this.world.getWorldBorder().contains(blockpos))
            {
                ...
                drawSelectionBoundingBox(iblockstate.getSelectedBoundingBox(this.world, blockpos).expandXyz(0.0020000000949949026D).offset(-d3, -d4, -d5), 0.0F, 0.0F, 0.0F, 0.4F);
            }
    ...
    {
        for (EntityLivingBase entitylivingbase : p_193054_1_.getEntitiesWithinAABB(EntityLivingBase.class, (new AxisAlignedBB(pos)).expandXyz(3.0D)))
        {
            entitylivingbase.func_191987_a(pos, p_193054_3_);
        }
    }
    ...
    public void func_190570_a(int id, boolean ignoreRange, boolean p_190570_3_, final double x, final double y, final double z, double xSpeed, double ySpeed, double zSpeed, int... parameters)
    {
        try
        {
