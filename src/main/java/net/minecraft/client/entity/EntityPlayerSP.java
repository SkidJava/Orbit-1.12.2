...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    /**
     * Used to tell if the thePlayer pressed forward twice. If this is at 0 and it's pressed (And they are allowed to
    ...
    public void onUpdate()
    {
        ...
        if (this.world.isBlockLoaded(new BlockPos(this.posX, 0.0D, this.posZ)))
        {
            ...
            else
            {
                ...
                EventPostMotionUpdates post = new EventPostMotionUpdates(this.rotationYaw, this.rotationPitch, this.onGround, this.posY);
                post.call();
            }
        }
    }
    ...
    /**
     * called every tick when the thePlayer is on foot. Performs all the things that normally happen during movement.
     */
    private void onUpdateWalkingPlayer()
    {
        EventPreMotionUpdates pre = new EventPreMotionUpdates(this.rotationYaw, this.rotationPitch, this.posX, this.posY, this.posZ);
        pre.call();
        ...
        if (this.isCurrentViewEntity())
        {
            ...
            {
                this.connection.sendPacket(new CPacketPlayer.PositionRotation(this.posX, y, this.posZ, yaw, pitch, this.onGround));
            }
            else if (flag2)
            {
                this.connection.sendPacket(new CPacketPlayer.Position(this.posX, y, this.posZ, this.onGround));
            }
            else if (flag3)
            {
    ...
    /**
     * Sends a chat message from the thePlayer.
     */
    public void sendChatMessage(String message)
    {
        if (!Managers.getManagers().commandManager.processCommand(message)) {
    ...
    {
        this.inventory.setItemStack(ItemStack.itemStack);
        super.closeScreen();
    ...
    {
        if (this.recipeBook.func_194076_e(p_193103_1_))
        {
            this.recipeBook.func_194074_f(p_193103_1_);
    ...
    public void addChatComponentMessage(ITextComponent chatComponent, boolean p_146105_2_)
    {
        ...
        {
            GuiIngame.getChatGUI().printChatMessage(chatComponent);
        }
    }
    ...
    {
        GuiIngame.getChatGUI().printChatMessage(component);
    }
    ...
    /**
     * Returns whether the entity is in a server theWorld
     */
    ...
    public void moveEntity(MoverType moverType, double x, double y, double z)
    {
        ...
        EventMove move = new EventMove(moverType, x, y, z);
        move.call();

        super.moveEntity(moverType, move.getX(), move.getY(), move.getZ());
