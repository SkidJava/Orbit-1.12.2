...
import net.minecraft.client.gui.inventory.GuiMerchant;
...
public class NetHandlerPlayClient implements INetHandlerPlayClient
{
    ...
    /**
     * Registers some server properties (gametype,hardcore-mode,terraintype,difficulty,thePlayer limit), creates a new
     * WorldClient and sets the thePlayer initial dimension
     */
    ...
    public void handleEntityVelocity(SPacketEntityVelocity packetIn)
    {
        ...
        if (entity != null)
        {
            ...
            double motionY = (double)packetIn.getMotionX() / 8000.0D;
            double motionZ = (double)packetIn.getMotionX() / 8000.0D;

            if (Minecraft.getMinecraft().thePlayer == entity) {
                EventPlayerVelocity playerVelocity = new EventPlayerVelocity(motionX, motionY, motionZ);
                playerVelocity.call();
                if (playerVelocity.isCancelled()) {
                    return;
                }

                motionX = playerVelocity.motionX;
                motionY = playerVelocity.motionY;
                motionZ = playerVelocity.motionZ;
            }
    ...
    /**
     * Handles the creation of a nearby thePlayer entity, sets the position and held item
     */
    ...
    /**
     * Updates which hotbar slot of the thePlayer is currently selected
     */
    ...
    /**
     * Locally eliminates the entities. Invoked by the server when the items are in fact destroyed, or the thePlayer is no
     * longer registered as required to monitor them. The latter  happens when distance between the thePlayer and item
    ...
    /**
     * Renders a specified animation: Waking up a thePlayer, a living entity swinging its currently held item, being hurt
    ...
    /**
     * Retrieves the thePlayer identified by the packet, puts him to sleep if possible (and flags whether all players are
     * asleep)
     */
    ...
    /**
     * Verifies that the server and client are synchronized with respect to the inventory/container opened by the thePlayer
    ...
    public void handleChangeGameState(SPacketChangeGameState packetIn)
    {
        ...
        else if (i == 5)
        {
            ...
            {
                GuiIngame.getChatGUI().printChatMessage(new TextComponentTranslation("demo.help.movement", GameSettings.getKeyDisplayString(gamesettings.keyBindForward.getKeyCode()), GameSettings.getKeyDisplayString(gamesettings.keyBindLeft.getKeyCode()), GameSettings.getKeyDisplayString(gamesettings.keyBindBack.getKeyCode()), GameSettings.getKeyDisplayString(gamesettings.keyBindRight.getKeyCode())));
            }
            else if (f == 102.0F)
            {
                GuiIngame.getChatGUI().printChatMessage(new TextComponentTranslation("demo.help.jump", GameSettings.getKeyDisplayString(gamesettings.keyBindJump.getKeyCode())));
            }
            else if (f == 103.0F)
            {
                GuiIngame.getChatGUI().printChatMessage(new TextComponentTranslation("demo.help.inventory", GameSettings.getKeyDisplayString(gamesettings.keyBindInventory.getKeyCode())));
            }
        }
        else if (i == 6)
        {
    ...
    public void func_191980_a(SPacketRecipeBook p_191980_1_)
    {
        ...
        recipebook = this.gameController.thePlayer.getRecipeBook();
     ...
     * thePlayer instance and finally "MC|RPack" which the server uses to communicate the identifier of the default server
    ...
    /**
     * Updates a team managed by the scoreboard: Create/Remove the team registration, Register/Remove the thePlayer-team-
    ...
    /**
     * Updates en entity's attributes and their respective modifiers, which are used for speed bonusses (thePlayer
    ...
    /**
     * Gets the client's description information about another thePlayer on the server.
     */
