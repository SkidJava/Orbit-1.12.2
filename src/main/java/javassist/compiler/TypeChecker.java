...
public class TypeChecker extends Visitor implements Opcode, TokenId
{
                               ...
                               ASTree right) throws CompileError
    {
        ...
        if (left instanceof javassist.compiler.ast.StringL && right instanceof javassist.compiler.ast.StringL && op == '+')
            newExpr = new StringL(((javassist.compiler.ast.StringL)left).get()
                                  + ((javassist.compiler.ast.StringL)right).get());
