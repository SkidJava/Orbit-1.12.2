...
public final class Parser implements TokenId
{
    ...
    private Stmnt parseBlock(SymbolTable tbl) throws CompileError
    {
        ...
        if (body == null)
        {
            return new Stmnt(BLOCK);    // empty blocks
        }
        else
        {
            return body;
    ...
    /* switch.statement : SWITCH "(" expression ")" "{" switch.blocks "}"
     *
     * swtich.blocks : ( switch.label statement* )*
     *
    ...
    private Stmnt parseSwitchBlock(SymbolTable tbl) throws CompileError
    {
        ...
        if (s == null)
        {
            throw new CompileError("empty switch blocks", lex);
        }

        int op = s.getOperator();

        if (op != CASE && op != DEFAULT)
            throw new CompileError("no case or default in a switch blocks",
                                   lex);
     ...
     *     SYNCHRONIZED "(" expression ")" blocks.statement
     */
    ...
    /* try.statement
     * : TRY blocks.statement
     *   [ CATCH "(" class.type Identifier ")" blocks.statement ]*
     *   [ FINALLY blocks.statement ]*
     */
