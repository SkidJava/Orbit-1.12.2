...
public class CharFontRenderer extends CharFont {
    protected CharFont.CharData[] boldChars = new CharFont.CharData[256];
    protected CharFont.CharData[] italicChars = new CharFont.CharData[256];
    protected CharFont.CharData[] boldItalicChars = new CharFont.CharData[256];

    private final int[] colorCode = new int[32];
    private final String colorcodeIdentifiers = "0123456789abcdefklmnor";

    public CharFontRenderer(Font font) {
        this(font, 0);
    }

    public CharFontRenderer(Font font, int kerning) {
        super(font, kerning);
    ...
    public float drawCenteredStringWithShadow(String text, float x, float y, int color) {
        float shadowWidth = drawString(text, x - getWidth(text) / 2 + 1.0f, y + 1.0f, color, true);
        return drawString(text, x - getWidth(text) / 2, y, color);
    }
    ...
    public float drawString(String text, float x, float y, int color, boolean shadow) {
        ...
        CharFont.CharData[] currentData = this.charData;
        ...
        if (render) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if ((character == '§') && (i < size)) {
                    ...
                    x += currentData[character].width - 8 + this.kerning;
                }
            }
    ...
    public int getWidth(String text) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if ((character == '§') && (i < size)) {
                ...
                width += currentData[character].width - 8 + this.kerning;
            }
        }
    ...
    private void setupBoldItalicIDs() {
        texBold = setupTexture(this.font.deriveFont(1), this.boldChars);
        texItalic = setupTexture(this.font.deriveFont(2), this.italicChars);
        texItalicBold = setupTexture(this.font.deriveFont(3), this.boldItalicChars);
    }
    ...
    public List<String> wrapWords(String text, double width) {
        ...
        if (getWidth(text) > width) {
            ...
            if (currentWord.length() > 0) if (getWidth(currentWord) < width) {
